...
public abstract class PartialHeader extends Header {
    ...
    public static void incompatibleHeaders() {
        ExtensionList.lookup(PartialHeader.class).stream().filter(h -> !h.isCompatible()).forEach(header -> {
            LOGGER.warning("%s:%s not compatible with %s".formatted(
            ...
            new AdministrativeError(
                    header.getClass().getName(),
                    "Incompatible Header",
                    "The plugin %s is attempting to replace the Jenkins header but is not compatible with this version of Jenkins. The plugin should be updated or removed.".formatted(
